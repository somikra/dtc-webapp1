...
const ForecastingTool = () => {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        if (!hasRequiredFields) {
          ...
          setFileData(result.data.map(row => ({
            ...row,
            date: new Date(row.date).toISOString().split('T')[0], // Normalize dates
  ...
  // Forecast functions with fixed logic
  const historicalTrend = (data, startDate, duration, range) => {
    const totalUnits = data.reduce((sum, row) => sum + row.quantity, 0);
    const totalSales = data.reduce((sum, row) => sum + row.sales, 0);
    const start = new Date(data[0].date);
    const end = new Date(data[data.length - 1].date);
    const days = Math.max(1, (end - start) / (1000 * 60 * 60 * 24) + 1); // Avoid division by zero
  ...
  const seasonalBoost = (data, startDate, duration, range) => {
    const totalUnits = data.reduce((sum, row) => sum + row.quantity, 0);
    const totalSales = data.reduce((sum, row) => sum + row.sales, 0);
    const days = Math.max(1, (new Date(data[data.length - 1].date) - new Date(data[0].date)) / (1000 * 60 * 60 * 24) + 1);
    const dailyAvgUnits = totalUnits / days;
    const dailyAvgSales = totalSales / days;
    const boostFactor = 1.2; // 20% boost
    return generateForecast(dailyAvgUnits, dailyAvgSales, startDate, duration, range, data, (date) => {
      ...
      const monthUnits = monthData.reduce((sum, row) => sum + row.quantity, 0);
      const avgMonthUnits = totalUnits / 12;
      return monthUnits > avgMonthUnits ? boostFactor : 1;
    });
  };
  ...
  const growthAggressive = (data, startDate, duration, range) => {
    ...
    const dailyAvgUnits = totalUnits / days;
    const dailyAvgSales = totalSales / days;
    const growthFactor = 2; // 2x multiplier
    const daysSinceLast = (startDate - new Date(data[data.length - 1].date)) / (1000 * 60 * 60 * 24);
    const adjustedGrowth = Math.min(growthFactor, 1 + (daysSinceLast / 365)); // Cap growth
    return generateForecast(dailyAvgUnits * adjustedGrowth, dailyAvgSales * adjustedGrowth, startDate, duration, range, data);
  };
  ...
  const productBreakout = (data, startDate, duration, range) => {
    ...
    return products.map(product => {
      ...
      const totalUnits = productData.reduce((sum, row) => sum + row.quantity, 0);
      const totalSales = productData.reduce((sum, row) => sum + row.sales, 0);
      const days = Math.max(1, (new Date(productData[productData.length - 1].date) - new Date(productData[0].date)) / (1000 * 60 * 60 * 24) + 1);
      const dailyAvgUnits = totalUnits / days;
      const dailyAvgSales = totalSales / days;
  ...
  const generateForecast = (dailyAvgUnits, dailyAvgSales, startDate, duration, range, data, boostFn = () => 1) => {
    ...
    for (let i = 0; i < duration; i++) {
      ...
      const periodSales = (dailyAvgSales * periodDays * boost).toFixed(2);
      forecast.push({
  ...
  const handleGenerateForecast = () => {
    ...
    setError('');
    try {
      const startDate = new Date('2025-03-18'); // Fixed for testing; adjust to dynamic input if needed
      const methodMap = {
